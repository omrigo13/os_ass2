...
exec(char *path, char **argv)
{
  ...
  p->sz = sz;
  cur_t->trapframe->epc = elf.entry;  // initial program counter = main
  cur_t->trapframe->sp = sp; // initial stack pointer
  proc_freepagetable(oldpagetable, oldsz);

  // task 2.1.2 back signal handlers to default not including SIG_IGN or SIG_DFL
  for(i = 0; i < 32; i++) {
